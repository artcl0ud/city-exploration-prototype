        ...
        private void ShowExtractOptim(float localstartX, float localstartY, float width, float height)
        {
            try
            {
                ...
                if (MidiPlayerGlobal.ImSFCurrent != null)
                {
                    ...
                    */
                    RemoveUnusedWaves = GUI.Toggle(new Rect(xpos, ypos, widthCheck, itemHeight), RemoveUnusedWaves, new GUIContent("Remove unused waves", "If check, keep only waves used by your midi files and/or in the selected banks"), MidiCommonEditor.styleToggle);
                    //xpos += widthCheck + espace;
                    ypos += itemHeight;

                    LogDetailSoundFont = GUI.Toggle(new Rect(xpos, ypos, widthCheck, itemHeight), LogDetailSoundFont, new GUIContent("Log SoundFont Detail", "If check, keep only waves used by your midi files"), MidiCommonEditor.styleToggle);
                    ypos += itemHeight;
